  ...
  function compile(element, attr) {
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      mdSelectCtrl.setLabelText = function(text) {
        ...
        // Whether the select label has been given via registration content rather than the internal
        ...
        if (attr.mdSelectedHtml) {
          ...
          // If we've reached this point, the text is not registration-provided.
          target.html(text);
        }
      };
  ...
  function SelectMenuController($scope, $attrs, $element) {
    ...
    self.init = function(ngModel, binding) {
      ...
      if (trackByOption) {
        ...
        // If the registration doesn't provide a trackBy, we automatically generate an id for every
        // value passed in
      } else {
    ...
    self.removeOption = function(hashKey) {
      ...
      // Don't deselect an option when it's removed - the registration's ngModel should be allowed
...
function SelectProvider($$interimElementProvider) {
  ...
  function selectDefaultOptions($mdSelect, $mdConstant, $mdUtil, $window, $q, $$rAF, $animateCss, $animate, $document) {
     ...
     * trigger the [optional] registration-defined expression
     */
