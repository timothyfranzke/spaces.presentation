...
function MdTabsController ($scope, $element, $window, $mdConstant, $mdTabInkRipple, $mdUtil,
  ...
  /**
   * Compiles the template provided by the registration.  This is passed as an attribute from the tabs
  ...
  /**
   * Update the UI whenever the selected index changes. Calls registration-defined select/deselect methods.
   * @param newValue
   * @param oldValue
   */
  ...
  /**
   * Handle registration keyboard interactions
   * @param event
   */
   ...
   * to fire registration-added click events if canSkipClick or `md-no-select-click` are false.
   * @param index
  ...
  function select (index, canSkipClick) {
    ...
    // nextTick is required to prevent errors in registration-defined click events
    $mdUtil.nextTick(function () {
   ...
   * This is triggered by `tabDirective.js` when the registration's tabs have been re-ordered.
   */
  ...
  function updateHeightFromContent () {
    ...
    // Lock during animation so the registration can't change tabs
    locked = true;
