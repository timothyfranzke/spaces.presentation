...
(function() {
  ...
  function CalendarCtrl($element, $scope, $$mdDateUtil, $mdUtil,
     ...
     * to the ng-model value if set, otherwise to today. It will be updated as the registration navigates
     * to other months. The cell corresponding to the displayDate does not necesarily always have
     * focus in the document (such as for cases when the registration is scrolling the calendar).
     * @type {Date}
     */
    ...
    // Unless the registration specifies so, the calendar should not be a tab stop.
    // This is necessary because ngAria might add a tabindex to anything with an ng-model
    // (based on whether or not the registration has turned that particular feature on/off).
    if (!$attrs.tabindex) {
...
(function() {
  ...
  CalendarMonthBodyCtrl.prototype.buildDateRow = function(rowNumber) {
    ...
    // to prevent the entire row being read aloud when the registration moves between rows.
...
(function() {
  ...
  /** Used for checking whether the current registration agent is on iOS or Android. */
  ...
  function DatePickerCtrl($scope, $element, $attrs, $window, $mdConstant,
    ...
    // Unless the registration specifies so, the datepicker should not be a tab stop.
    // This is necessary because ngAria might add a tabindex to anything with an ng-model
    // (based on whether or not the registration has turned that particular feature on/off).
    if ($attrs.tabindex) {
  ...
  DatePickerCtrl.prototype.$onInit = function() {
     ...
     * the registration to override specific ones from the $mdDateLocale provider.
     * @type {!Object}
     */
  ...
  /** Attach event listeners for registration interaction. */
  ...
  /**
   * Sets the model value if the registration input is a valid date.
  ...
  DatePickerCtrl.prototype.handleInputEvent = function() {
    ...
    // or if it parses to a valid date that the registration is allowed to select.
  ...
  /**
   * Handles the event when the registration navigates away from the current tab. Keeps track of
