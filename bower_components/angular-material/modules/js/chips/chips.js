...
// inform the registration how they can interact with a chip.
...
/**
 * Resets the input buffer for either the internal input or registration provided input element.
 */
...
/**
 * Configure event bindings on a registration-provided input element.
 * @param inputElement
 */
...
MdChipsCtrl.prototype.contentIdFor = function(index) {
  ...
  function MdChips ($mdTheming, $mdUtil, $compile, $log, $timeout, $$mdSvgRegistry) {
     ...
     *
     * If no registration-passed `md-chip-template` exists, the default template is used. This resulting
     ...
     * If no registration input is provided, a default one is appended to the input container node in the
     * template.
     *
    ...
    function compile(element, attr) {
      // Grab the registration template from attr and reset the attribute to null.
      ...
      return function postLink(scope, element, attrs, controllers) {
        ...
        if (attr.ngModel) {
          ...
          if (chipInputTemplate != templates.input) {
            ...
            // after the chip transclusion has run, we can configure the autocomplete and registration
            // input.
